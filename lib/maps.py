...
def plotMap(lat, lon, data, title, graphicName, plotRange = None, units='', colorScheme = 'viridis', projectionSelected = ccrs.PlateCarree()):
    ...
    fig = plt.figure( figsize = (16, 9) ) # setup size/aspect ratio of the figure
    ax = plt.axes( projection = projectionSelected ) # make a cartopy axes
    ax.coastlines(resolution='110m', color='black') # plot coastlines on map
    ax.set_global() # show the whole earth
    ax.gridlines() # put lat/lon markers
    ...
    ax.scatter(lon, lat, c=data, cmap = colorScheme, norm=norm, s=2, marker="o", edgecolors='none', transform=ccrs.PlateCarree())
